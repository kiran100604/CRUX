...
- [x] Contradiction-aware writes: each new fact is checked against its embedding neighborhood at write time (Haiku-judged with a key, similarity offline), candidates persisted in a `conflicts` table; resolve by supersede or dismiss (sticks).
- [x] Async ingestion: `/ingest` saves the episode synchronously and extracts facts in a background worker (own DB connection, WAL) — big documents never block the request. CLI ingest stays synchronous (one-shot process).
- [x] Re-embed on edit: `crux edit` / dashboard edit / refine-on-promote all re-embed and re-run contradiction checks, so search never goes stale on edited facts.
